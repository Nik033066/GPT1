        ...
        for step_i in range(cfg.max_steps):
            ...
            if len(res.steps) >= 3:
                ...
                if all(a.action == last_3[0].action and a.selector == last_3[0].selector for a in last_3):
                    self._mem.add("SYSTEM WARNING: Hai ripetuto la stessa azione 3 volte. CAMBIA STRATEGIA ORA.")
                    print("⚠️ Loop rilevato: inserito warning in memoria.")
            ...
            try:
                ...
                msg = f"Timeout pianificazione dopo {cfg.plan_timeout_ms}ms"
                res.answer = msg
                res.steps.append(Step(act=Act(action="done", text=msg), obs=obs.model_copy()))
                break
            dt = time.perf_counter() - t0
            print(f"⏱️ pianificazione {dt:.1f}s")

            if act.thought:
                print(f"🧠 {act.thought}")

            act_str: str = str(act.action)
            if act.selector:
                act_str += f" on '{act.selector}'"
            if act.text:
                act_str += f" text='{act.text}'"
            if act.url:
                act_str += f" to '{act.url}'"
            print(f"⚡ {act_str}")

            logger.debug(f"DEBUG: act={act.action} url={act.url}")
...
async def _move_cursor_to(br: Br, cur: Cur, selector: str) -> bool:
    await br.set_status("Muovo il cursore...")
    ...
    screen_offset = await br.get_screen_offset()
    
    step_count = 0
    for x, y in cur.move(cx, cy, w):
        await br.mouse_move_physical(x, y, offset=screen_offset)
        step_count += 1
    
    await br.mouse_move_physical(cx, cy, offset=screen_offset)
    return True


async def _handle_action(act: Act, obs: Obs, mem: Mem, br: Br, cur: Cur, res: RunRes) -> bool:
    """Gestisce una singola azione pianificata. Ritorna True se l'esecuzione deve fermarsi."""
    if act.action == "done":
        if act.text:
            res.answer = act.text
        else:
            res.answer = obs.text[:1200]
    ...
    if act.action == "navigate" and act.url:
        ...
        if "/search?q=" in act.url:
            mem.add("qnav")
        if "duckduckgo.com/?q=" in act.url:
            mem.add("duckdone")
        ...
        mem.add("SYSTEM WARNING: press richiede key (es. Enter).")
        ...
        if not found:
            ...
            if bb2 is not None:
                cx2, cy2, _ = bb2
                await br.click_physical(cx2, cy2)
...
async def _run_goal(goal: str, cfg: Cfg, llm: LLM, br: Br) -> RunRes:
    ...
    try:
        ...
        res = RunRes(goal=goal, answer=f"Errore: {e}")
        return res
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    cfg = Cfg(
        ...
        os_cursor=args.os_cursor if args.os_cursor is not None else base.os_cursor,
    ...
    try:
        ...
        if str(e) == "hf_auth_required":
             logger.error(
                f"ERRORE AUTORIZZAZIONE per {cfg.model_id}.\n"
                "1. Vai su: https://huggingface.co/google/gemma-3-1b-it\n"
                "2. Accetta la licenza (Agree and Access).\n"
                "3. Assicurati di aver fatto login: `huggingface-cli login`\n"
                "   o imposta la variabile HF_TOKEN."
            )
             return 2
        raise
    ...
    try:
        try:
            ...
            if "system_browser_not_found" in str(e) or "system_browser_connection_failed" in str(e):
                if cfg.browser == "system" or cfg.browser is None:
                     print("⚠️  Browser di sistema non trovato o non accessibile.")
                     print("🔄  Avvio browser integrato (Playwright)...")
                     await br.stop()
                     br = Br(
                        timeout_ms=cfg.page_timeout_ms,
                        browser="playwright",
        ...
        if args.goal:
            ...
            # Modalità chat
            print(f"🤖 Agente attivo ({cfg.model_id}). Scrivi cosa vuoi fare (o 'exit' per uscire).")
            session = AgentSession(llm=llm, cfg=cfg)
            while True:
                try:
                    goal = input("\n👉 Tu: ").strip()
                ...
                if out.steps:
                    last = out.steps[-1]
                    if last.act.action == "navigate":
                        print(f"   (Navigato a: {last.act.url})")
                    
    except RuntimeError as e:
        if str(e) == "playwright_missing":
            logger.error("Playwright non installato. Installa con `python3 -m pip install playwright`")
            return 3
        if str(e) == "playwright_browsers_missing":
            logger.error("Browser Playwright mancanti. Esegui `python3 -m playwright install chromium`.")
            return 4
        if "system_browser_connection_failed" in str(e):
            logger.error("Impossibile connettersi al browser di sistema.")
            logger.error("Assicurati di aver chiuso completamente il browser (Chrome/Brave/Edge) e riprova.")
            logger.error("L'agente proverà a riaprirlo in modalità debug.")
            return 5
        raise
    finally:
        await br.stop()
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p.add_argument("goal", type=str, nargs="?", help="Obiettivo (opzionale)")
    p.add_argument("--hf", action="store_true")
    p.add_argument("--browser", choices=["playwright", "system", "cdp"], default=None)
    ...
    p.add_argument("--os-cursor", action=argparse.BooleanOptionalAction, default=None)
